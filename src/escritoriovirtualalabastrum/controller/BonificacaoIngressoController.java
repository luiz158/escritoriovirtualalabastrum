...
public class BonificacaoIngressoController {
	...
	private List<BonificacaoAuxiliar> calcularBonificacoes(Usuario usuario, Integer ano, Integer mes) {
		...
		if (usuario.getPosAtual().toLowerCase().contains(MalaDiretaService.DIAMANTE.toLowerCase())) {
			...
			calcularBonificacoesVariaveisDeDiamante(usuario, ano, mes, malaDiretaCompletaHash);
		}

		return bonificacoes;
	}

	private void calcularBonificacoesVariaveisDeDiamante(Usuario usuario, Integer ano, Integer mes, TreeMap<Integer, MalaDireta> malaDiretaCompletaHash) {
